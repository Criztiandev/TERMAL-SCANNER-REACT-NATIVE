...
const Activation = () => {
  ...
  const setTempActivation = useSetAtom(humidityActivationAtom);
  const connection = useAtomValue(connectionAtom);

  useQuery({
    queryFn: async () => {
      if (connection === "OFF") return false;
      const result = await repository.read("humidity/active");
      setIsActive(result);
      return result;
    },
    queryKey: ["humidity-activation1"],
  });

  const activateMutation = useMutation({
    mutationFn: async () => {
      return await repository.update("/humidity", {
        active: !isActive,
      });
    },
    mutationKey: ["humidity-active"],
    onSuccess: () => {
...
const TimerDeactivation = () => {
  ...
  const setTimerAtom = useSetAtom(humidityTimerAtom);
...
const DetailScreen = () => {
  const activation = useAtomValue(humidityActivationAtom);
  ...
  useQuery({
    queryFn: async () => {
      ...
      const result = await respository.read(`humidity/week/${currentDay}`);
