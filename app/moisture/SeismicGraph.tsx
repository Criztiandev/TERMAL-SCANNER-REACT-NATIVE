...
const Activation = () => {
  ...
  const setTempActivation = useSetAtom(moistureActivationAtom);
  const connection = useAtomValue(connectionAtom);

  useQuery({
    queryFn: async () => {
      if (connection === "OFF") return false;
      const result = await repository.read("moisture/active");
      setIsActive(result);
      return result;
    },
    queryKey: ["moisture-activation"],
  });

  const activateMutation = useMutation({
    mutationFn: async () => {
      return await repository.update("/moisture", {
        active: !isActive,
      });
    },
    mutationKey: ["moisture-active"],
    onSuccess: () => {
...
const TimerDeactivation = () => {
  ...
  const setTimerAtom = useSetAtom(moistureTimerAtom);
...
const DetailScreen = () => {
  const activation = useAtomValue(moistureActivationAtom);
  ...
  const renderStatus = !activation ? (
    <View className="px-4 py-2 rounded-full w-[100px] border border-gray-300 bg-gray-100  justify-center items-center mx-auto">
      <Text className="font-bold text-[18px]">Inactive</Text>
    </View>
  ) : currentData[currentData.length - 1] <= 5 ? (
    <View className="px-4 py-2 rounded-full w-[100px] border border-orange-300 bg-orange-100  justify-center items-center mx-auto">
      <Text className="font-bold text-[18px]">Dry</Text>
    </View>
  ) : (
    <View className="px-4 py-2 rounded-full w-[100px] border border-blue-300 bg-blue-100  justify-center items-center mx-auto">
      <Text className="font-bold text-[18px]">Wet</Text>
    </View>
  );
